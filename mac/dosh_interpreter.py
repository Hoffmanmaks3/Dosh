...
class DoshInterpreter:
    ...
    def set_text_color(self, color: str):
        ...
        if not IS_WINDOWS:
            # Use ANSI escape codes for macOS/Linux
            ansi_colors = {
                'black': '\033[30m',
                'red': '\033[31m',
                'green': '\033[32m',
        ...
        if color in color_map:
            ...
            try:
                ...
                b = int(color[4:6], 16)
                if r > g and r > b:
                    self.kernel32.SetConsoleTextAttribute(self.std_output_handle, 4)
                elif g > r and g > b:
                    self.kernel32.SetConsoleTextAttribute(self.std_output_handle, 2)
                elif b > r and b > g:
                    self.kernel32.SetConsoleTextAttribute(self.std_output_handle, 1)
                elif r > 200 and g > 200 and b > 200:
                    self.kernel32.SetConsoleTextAttribute(self.std_output_handle, 7)
                else:
                    self.kernel32.SetConsoleTextAttribute(self.std_output_handle, 7)
            except:
                pass
    ...
    def is_key_pressed(self, key: str) -> bool:
        """Check if a specific key is currently pressed (Windows only, returns False on other platforms)"""
        if not IS_WINDOWS:
            # Key detection not supported on macOS/Linux in terminal
            return False
